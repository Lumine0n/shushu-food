...
export default function MePage() {
  ...
  return <AppFrame><main className="page"><PageHeader eyebrow="My table" title={`${currentUser.nickname}的饭桌`} description={dataMode === "demo" ? "当前数据保存在这个浏览器中。配置 Supabase 后即可与朋友共享。" : "你吃过的每一口，都会让下次推荐更准。"} />
    ...
    <section><h2 className="serif mb-3 text-xl font-bold">我的收藏</h2>{favoriteFoods.length ? <div className="grid gap-3">{favoriteFoods.map((food) => { const place = places.find((item) => item.id === food.placeId); return <Link className="soft-card flex items-center justify-between gap-3 p-4" href={`/food/${food.id}`} key={food.id}><div><strong>{food.name}</strong><p className="mt-1 text-xs text-[var(--muted)]">{food.tags.slice(0, 3).join(" · ")}</p></div><span className="font-extrabold text-[var(--red)]">{food.priceCents != null ? formatPrice(food.priceCents) : place?.averagePriceCents != null ? `人均 ${formatPrice(place.averagePriceCents)}` : "价格未知"}</span></Link>; })}</div> : <div className="soft-card p-6 text-center text-sm text-[var(--muted)]">还没有收藏，遇到想吃的先留个记号。</div>}</section>
