...
export default function FoodDetailPage() {
  ...
  const { loading, foods, places, experiences, favorites, toggleFavorite, isAuthenticated, requestLogin } = useAppStore();
  ...
  return <AppFrame><main className="page"><PageHeader back={`/place/${place.id}`} eyebrow={food.mealType} title={food.name} description={food.description} />
    ...
    <section className="soft-card p-4"><div className="flex items-start justify-between gap-4"><div><p className="flex items-center gap-1.5 font-extrabold"><MapPin size={17} color="#c94a32" />{place.name}</p><p className="mt-1 text-sm text-[var(--muted)]">{place.address}</p>{place.coordinateStatus === "estimated" ? <p className="mt-2 text-xs font-bold text-[#9a6c25]">位置为地址估算，等待高德校准</p> : null}</div><strong className="text-right text-lg text-[var(--red)]">{food.priceCents != null ? formatPrice(food.priceCents) : place.averagePriceCents != null ? `人均 ${formatPrice(place.averagePriceCents)}` : "价格未知"}</strong></div>{place.notes ? <p className="mt-4 rounded-xl bg-[#f7f3e8] px-3 py-2 text-sm leading-6 text-[var(--muted)]">{place.notes}</p> : null}<div className="mt-4 flex flex-wrap gap-2">{food.tags.map((tag) => <span className="rounded-full bg-[#eef2ec] px-3 py-1.5 text-xs font-bold text-[var(--green)]" key={tag}>{tag}</span>)}</div><div className="mt-5 grid grid-cols-2 gap-2"><button className="secondary-button flex items-center justify-center gap-2" onClick={() => toggleFavorite(food.id)}><Heart size={18} fill={favorite ? "#c94a32" : "none"} color={favorite ? "#c94a32" : "currentColor"} />{favorite ? "已收藏" : "收藏"}</button><button className="primary-button flex items-center justify-center gap-2" onClick={async () => { if (!isAuthenticated) { requestLogin(); return; } let token = `demo-${food.id}`; if (process.env.NEXT_PUBLIC_SUPABASE_URL) { const response = await fetch("/api/share", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ foodId: food.id }) }); if (!response.ok) { alert("分享链接创建失败，请稍后重试"); return; } token = (await response.json()).token; } const url = `${window.location.origin}/share/${token}`; await navigator.clipboard.writeText(url); alert("分享链接已复制"); }}><Share2 size={18} />分享</button></div></section>
